...
export default function Signup(){
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (message === "success") {
        navigate("/login");
      } else if (message === "Already Exists Error:(") {
        setError({ email: "User Already Exist", password: "" });
      } else {
    ...
    return(
        <div className="w-screen h-screen flex font-open box-border p-5 bg-no-repeat bg-cover bg-center" style={{backgroundImage:`url('${earth}')`}}>
           <div className="w-full h-full backdrop-blur bg-black/20 box-border p-10 rounded-lg  m-auto text-center flex flex-col">
                <h1 className="uppercase text-white font-bold text-2xl mt-auto my-6">Signup</h1>
                <form className="flex flex-col text-left mx-auto my-2" onSubmit={handleSubmit}>
                <input type="text"  name="name" value={formData.name} onChange={handleInputChange} className="px-2 w-80 py-3 outline-none bg-white/40  mb-6 placeholder:text-gray-800 rounded-r border-l-4 border-green-600 text-sm" required placeholder="Username"/>
                ...
                <button type="submit" className="w-80 rounded-full mt-5 text-lg bg-green-600 text-white py-2 uppercase tracking-wider font-bold">Submit</button>
                </form>
                <p className="text-xs mb-auto text-white font-semibold">Don't have an account? <Link to="/login" className="text-green-600 font-bold">Sign Up</Link></p>
            </div>
